...
- **§6 nonlinearity check** (`scripts/fit_nonlinearity_check.py`): pooled model with per-rider PM2.5 quartile dummies vs. the linear term — AIC essentially tied (1261.3 linear vs. 1262.3 quartile), no quartile effect individually clears zero, no monotonic trend. No evidence of a threshold/saturation effect in the average, though this check inherits the same pooled/common-effect limitation as §5's Model A (see caveat in the script's output) — it can't rule out nonlinearity in any individual rider's own dose-response, only in the pooled average.
