...
- **§4 primary (per-rider + random-effects meta-analysis)**: SEs clustered by `test_date` (riders take 2-3 PVT sessions/shift, so same-day sessions aren't independent — clustering matters: it moved the pooled result from non-significant to significant). Pooled β = **−0.00064 Hz/µg/m³** (95% CI [−0.00125, −0.00003], z=−2.05, p=0.040, **significant at α=0.05** under the DerSimonian-Laird τ² estimator + Wald-z inference used here), I²=44.8% (real cross-rider heterogeneity — most riders trend negative, a few, notably `Panatda_6018`, trend clearly positive). **Caveat**: the point estimate is robust to τ² estimator choice, but the significance boundary is close — see the tau² estimator sensitivity check below (p ranges 0.038–0.093 across τ² estimator × Wald-z/HKSJ combinations). Read as suggestive-but-not-robust rather than either a clean pass or a null result.
...
- **§6 tau² estimator sensitivity check** (`scripts/fit_tau2_estimator_check.py`): the primary analysis pools per-rider estimates using DerSimonian-Laird (DL) with Wald-z inference. DL is a closed-form τ² estimator known to be biased for small k (k=22 here); Wald-z is likewise a large-k approximation. Re-pooled the same 22 per-rider estimates under Paule-Mandel (PM) and REML (the alternatives PLAN.md §4 flagged as acceptable), and additionally checked Hartung-Knapp-Sidik-Jonkman (HKSJ) t(k-1) inference, the standard small/moderate-k-appropriate correction to Wald-z (IntHout et al. 2014).
  - **Point estimate is stable**: β ranges only −0.00063 to −0.00064 (1.9% spread) across DL/PM/REML — the effect itself does not depend on τ² method.
  - **τ² differs more**: DL=7.0e-7, REML=6.8e-7 (close, agree within ~0.1% across three independent computation routes: bounded optimizer, log-spaced grid scan, Fisher-scoring iteration), PM=1.15e-6 (~65% larger than DL) — DL/PM also cross-checked against `statsmodels.stats.meta_analysis.combine_effects` (matches to 5+ significant figures). (PM's own HKSJ scale-correction factor comes out to exactly 1 by construction — independent internal confirmation of both the PM and HKSJ implementations.)
  - **p across all six (τ² estimator × Wald-z/HKSJ) combinations ranges 0.038–0.093** — every combination lands in the borderline band around α=0.05. The split is not "only the primary analysis's exact specification clears 0.05": **DL+Wald-z (p=0.040, the primary analysis's own specification) and REML+Wald-z (p=0.038, actually the lowest p of the six) are both significant**, since DL and REML's τ² estimates agree closely (within ~3% of each other); Paule-Mandel's τ² is ~65% larger and keeps it non-significant under Wald-z (p=0.078) and HKSJ (p=0.093) alike; switching to HKSJ inference (t vs. z, the more appropriate choice at k=22) pushes all three below significance (p=0.069–0.093). So the boundary-crossing tracks two real, independent axes — τ² estimator choice (DL/REML vs. PM) and inference method (Wald-z vs. HKSJ) — not one fragile pairing.
  - **Bottom line**: the result is **suggestive but not robustly significant at α=0.05** — it sits right at the boundary under every τ²-estimator/inference-method combination tried, rather than clearing it comfortably or failing it clearly. The primary analysis's Wald-z/DL p=0.040 should not be read as the last word; report the HKSJ interval alongside it (HKSJ is the better-calibrated choice at k=22). This reads consistently with the learning-effect check (§6, also boundary-sensitive with a stable point estimate) and should be weighed against the window-sensitivity and precision-sensitivity checks (§6), both of which found the effect *strengthening* under alternative specifications — taken together this looks like a real but modest effect, not a Wald-z artifact.
...
- **tau² estimator sensitivity check** (added after the original §6 list, per 2026-08-07 discussion): test whether the primary DerSimonian-Laird + Wald-z pooling is robust to τ² estimator choice and to small-k inference assumptions. Done — see above. **Result: point estimate is robust; significance is borderline (p=0.038–0.093) across every τ² estimator × Wald-z/HKSJ combination tried** — should be reported alongside the primary result (with the HKSJ interval), not treated as a minor footnote.
